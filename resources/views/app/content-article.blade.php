...
<body class="index-page">
    ...
    <div class="container mt-5 pt-5">
        <div class="card border shadow rounded-4">
            <div class="card-body">
                <h1 class="card-title h3 text-danger text-center fw-bold mb-3">{{ $article->title }}</h1>

                <p class="text-muted mb-3">
                    Oleh <strong>{{ $article->author }}</strong> —
                    <em>{{ $article->created_at->format('d M Y') }}</em>
                </p>

                <a href="{{ route('app.content') }}" class="btn-danger mb-4">
                    ← Kembali ke Daftar Artikel
                </a>

                @if ($article->content)
                    <div class="ratio ratio-16x9 mb-3" style="height: 600px;">
                        <iframe src="{{ asset('storage/' . $article->content) }}" type="application/pdf" width="100%"
